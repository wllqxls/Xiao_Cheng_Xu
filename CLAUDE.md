...
- `scripts`：本地验证和辅助脚本，只放项目内开发工具，不放游戏运行时代码。
...
- `scripts/*.mjs`：Node 本地验证脚本，必须可重复运行，输出用于检查开发状态。
...
- 地图主风格固定为“手绘航海世界地图 + 桌游式区域规则”，不要改回科技雷达/监控屏风格。
- 地图必须保留海洋、陆地、岛链、航线、城市、森林、农田、工业、港口、机场等多元区域表达。
- 侵蚀表现优先用陆地红点、红斑、半透明覆盖逐级加深；飞机和船只只作为传播风险线索，不做现实疾病图标。
